...
export default function Page1() {
  ...
  return (
    <>
      {/* Reading Canvas */}
      <div className="flex-1 overflow-y-auto px-10 py-10 max-w-4xl mx-auto scrollbar-hide">
        {/* Title & Header Info */}
        <div className="mb-10">
          <div className="flex items-center gap-3 mb-4">
            <span className="px-3 py-1 bg-secondary-container text-on-secondary-container rounded-full text-xs font-medium">深度阅读</span>
            <span className="text-sm text-outline">预计阅读时间: {Math.max(1, Math.ceil(doc.content.length / 300))}分钟</span>
          </div>
        ...
        {/* Cover Image */}
        <div className="mb-10 rounded-2xl overflow-hidden bg-surface-container-high aspect-video relative">
          <img 
        ...
        {doc.summary && doc.summary.length > 0 && (
          <section className="mb-12 p-8 rounded-xl bg-gradient-to-br from-primary/5 to-primary-container/10 border border-primary/5 relative overflow-hidden group">
        ...
        {/* Article Content */}
        <article className="text-lg text-on-surface-variant leading-relaxed space-y-6 whitespace-pre-wrap">
          {doc.content}
        </article>

        {/* My Notes Editor */}
        <section className="mt-16 pt-10 border-t border-surface-container-high">
          ...
          <div className="bg-surface-container-lowest rounded-xl p-6 border border-outline-variant/20 shadow-sm focus-within:ring-2 focus-within:ring-primary/10 transition-all">
            <textarea 
              className="w-full min-h-[120px] bg-transparent border-none focus:ring-0 p-0 text-on-surface-variant placeholder:text-outline/40 leading-relaxed outline-none resize-none" 
              placeholder="输入你对这篇文章的思考或读书笔记..."
            ></textarea>
            <div className="flex items-center gap-4 mt-4 pt-4 border-t border-surface-container">
              ...
              <button 
                ...
                className="ml-auto px-6 py-2 bg-primary text-on-primary rounded-full text-sm font-medium hover:opacity-90 transition-opacity"
              >
                保存笔记
              </button>
            </div>
      ...
      {/* Right Sidebar */}
      <aside className="w-80 border-l border-surface-container-high bg-surface-container-low/50 p-8 space-y-10 hidden xl:block overflow-y-auto scrollbar-hide">
        {/* Metadata */}
        <div>
          ...
          <div className="space-y-4">
            <div className="flex items-center justify-between">
              ...
              <span className="text-sm font-medium text-primary">内部知识库</span>
            </div>
            <div className="flex items-center justify-between">
              <span className="text-sm text-on-surface-variant">创建日期</span>
              <span className="text-sm font-medium text-on-surface">{doc.createdAt}</span>
            </div>
            <div className="flex items-center justify-between">
              <span className="text-sm text-on-surface-variant">知识分类</span>
              <span className="text-xs px-2 py-1 bg-surface-container-highest rounded text-on-surface-variant font-medium">{doc.category}</span>
            </div>
          </div>
        </div>
        ...
        <div>
          <h4 className="text-xs font-bold text-outline uppercase tracking-widest mb-6">关联推荐</h4>
          <div className="space-y-6">
            {relatedDocs.map(relatedDoc => (
              ...
              >
                <p className="text-xs text-primary font-medium mb-1">来自 关联标签 #{relatedDoc.tags?.[0] || '默认'}</p>
            ...
            <a className="group block" href="#">
              <div className="bg-surface-container-highest p-4 rounded-lg flex items-center gap-3 hover:bg-surface-variant transition-colors">
                <Link2 className="text-outline" size={18} />
                <span className="text-xs text-outline font-medium">手动关联更多内容...</span>
              </div>
            </a>
          </div>
        </div>
